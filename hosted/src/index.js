...
const { resolveProjectFromAuth, errorShape } = require("./db");
...
app.use(async (req, res, next) => {
  ...
  try {
    ...
    if (!project) {
      return res.status(401).json(errorShape("unauthorized", "Missing or invalid API key", req.requestId));
    }
    req.project = project;
    next();
  } catch (err) {
...
app.post("/v1/billing/checkout", async (req, res, next) => {
  try {
    ...
    if (err.status === 501) {
      return res.status(501).json(errorShape(err.message, "Billing provider not configured", req.requestId));
    }
    next(err);
  }
});
...
app.use((err, _req, res, _next) => {
  ...
  res.status(500).json(errorShape("internal_error", "Unexpected server error", requestId, true));
});
